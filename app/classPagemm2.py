...
class MakePDF():
    ...
    def create_text(self, language):
        if ("text_ru" not in self.param.keys()):
            language == 'en'
            text = self.param["text"].replace('\t', ' ') + '\v'
        if ("text_ru" in self.param.keys()):
            text = self.param["text_ru"] + '\v'
            language == "ru"
            # lst=[]
    ...
    def nnew_page(self):
        ...
        if self.right_down.y > start_point_y:  # no first time start function nnew_page
    ...
    def cickle_while(self, ):
        ...
        if ('image' in self.param.keys()):
            ...
            if self.right_down.y > float(self.height_page + 10):
                self.nnew_page()
                self.left_up = fitz.Point(
                    self.param['bbox'][0], self.param['bbox'][1] + 2* self.offset - self.offset_last_page)
                self.right_down = fitz.Point(
                    self.param['bbox'][2], self.param['bbox'][3] + 2* self.offset - self.offset_last_page)
...
for tgg in doc.pages(start=86, stop=100):
    # tg = Pagem(doc[86])
    tg = Pagem(tgg)
    list_param = TakeList(listt=tg)

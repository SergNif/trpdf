...
class MakePDF():
    ...
    def nnew_page(self):
        ...
        self.right_down.x, self.right_down.y = self.param[1]["bbox"][2], self.p.y + self.param[1]["bbox"][3] - self.param[1]["bbox"][1]
    ...
    def cickle_while(self, ):
        ...
        for language in self.language:  # ["en", "ru"]:
            ...
            if 'img' not in (str(self.param[0]) if isinstance(self.param[0], int) else self.param[0]):
                while not 5 < rc < 10:
                    print(f"while verify height {self.right_down.y + float(self.param[1]['bbox'][3] - self.param[1]['bbox'][1]) + 20=}")
                    print(f'{self.width_page=}   {self.height_page=}')
                    print(f"while verify height {float(self.height_page)=}")
                    if self.right_down.y + float(self.param[1]['bbox'][3] - self.param[1]['bbox'][1]) + 20> float(self.height_page ):
                        # ls = count_coordinates_nnew_page()
                        # count_coordinates_nnew_page()
                        self.nnew_page()
                        # self.p = ls[0]
...
def translate(name_file_pdf: str):
    ...
    # ditt = {}
    # pag = Pagem()
    for pagge in doc_orig.pages(start=14, stop=20):
        ditt = {}

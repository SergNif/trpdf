...
class Singleton(object):
  _instances = {}
  def __new__(class_, param: list, docum: Document, width: float, height: float):
    if class_ not in class_._instances:
        class_._instances[class_] = super(Singleton, class_).__new__(class_, param, docum, width, height)
...
class MakePDF():
    ...
    def __init__(self, param: list, docum: Document, width: float, height: float):
        ...
        # vars(sys.modules[__name__])[type(self).__name__] = lambda: self, param, docum, width, height # singletonify
        self.docum = docum
        self.params = param
        self.param = []
        self.offset = 0.0
    ...
    def nnew_page(self):
        ...
        if self.right_down.y > 100:  # no first time start function nnew_page
            self.offset_last_page = self.left_up.y - self.offset - start_point_y
        self.offset = 0.0
        self.page = doc_new.new_page(
            width=self.width_page, height=self.height_page)
        font = fitz.Font("figo")
        ...
        self.right_down = fitz.Point(
            self.width_page - 30, start_point_y + self.fontsize/3)
    ...
    def cickle_while(self, ):
        ...
        self.left_up = fitz.Point(
            self.param['bbox'][0], self.param['bbox'][1] + self.offset - self.offset_last_page)
        self.right_down = fitz.Point(
            self.param['bbox'][2], self.left_up.y + self.fontsize/4)
        ...
        if ('image' in self.param.keys()):
            self.left_up = fitz.Point(
                self.param['bbox'][0], self.param['bbox'][1] + self.offset - self.offset_last_page)
            self.right_down = fitz.Point(
                self.param['bbox'][2], self.param['bbox'][3] + self.offset - self.offset_last_page)
        ...
        if ('image' not in self.param.keys()):
            ...
            if 'text_ru' in self.param.keys():
                self.offset += self.param["bbox"][3] - \
                    self.param["bbox"][1] + add_right_down_y
            print(
...
for tgg in doc.pages(start=86, stop=92):
    # tg = Pagem(doc[86])
    tg = Pagem(tgg)

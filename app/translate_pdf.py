        ...
        for n in klc[0:len(klc)-1]:
            ...
            if len(txt) > 0:
                d["number"] = [d1, d2, {"text_en": txt_en},  {"text_ru": txt_ru}, {"width_rect": len_stroke}, {
                    ...
                    "height_rect": float(height_stroke * self.fontsize * 1.31 + add_height),}
...
class MakePDF():
    ...
    def make_rect(self, left_upx, left_upy, right_downx, right_downy):
        if left_upy >= right_downy:
            self.right_down.y += (left_upy - right_downy)+ add_height
    ...
    def make_paragraph(self, language):
        ...
        rect = self.make_rect(left_upx= self.left_up.x, left_upy= self.left_up.y,
                             right_downx= self.right_down.x , right_downy= self.right_down.y)
    ...
    def nnew_page(self):
        ...
        self.right_down.x, self.right_down.y = float(self.width_page), float(self.p.y + add_height)
    ...
    def cickle_while(self, ):
        ...
        for language in self.language:  # ["en", "ru"]:
            ...
            if 'img' not in (str(self.param[0]) if isinstance(self.param[0], int) else self.param[0]):
                while not 5 < rc < 10:
                    if self.right_down.y + float(self.param[5]['height_rect'])> float(self.height_page + 150):
                        # ls = count_coordinates_nnew_page()
                        # count_coordinates_nnew_page()
                        self.nnew_page()
                        # self.p = ls[0]
                    ...
                    if rc <= 5:
                        ...
                        self.right_down.y += rc*(-1) if rc < -2 else 2.0
                    if rc >= 10:
    ...
    def start(self):
        ...
        for self.param in self.params:
            ...
            if docnew.page_count == 0:
                ...
                self.right_down = fitz.Point(self.width_page, point_right_down.y+4*add_height)
            self.cickle_while()
...
def translate(name_file_pdf: str):
    ...
    s3 = '_translate'
    name_file_pdf_translate = name_file_pdf[:-4] + f'{s3}' + name_file_pdf[-4:] 
    print(f'start translate {name_file_pdf} {name_file_pdf_translate}')
    # shutil.move('oldname', 'renamedfiles/newname')
    doc_orig = fitz.open("/home/serg/python_proj/fastapi/uploadfiles/Android.pdf")#(name_file_pdf)
    ...
    # ditt = {}
    # pag = Pagem()
    for pagge in doc_orig.pages(start=1, stop=100):
        ditt = {}

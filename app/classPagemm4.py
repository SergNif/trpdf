...
class Translator:  # класс перевод текста
    def __init__(self, source_lang: str, dest_lang: str) -> None:
        self.model_name = f'Helsinki-NLP/opus-mt-{source_lang}-{dest_lang}'
        self.model = MarianMTModel.from_pretrained(self.model_name)
        self.tokenizer = MarianTokenizer.from_pretrained(self.model_name)

    def delete_symbols(self, text:str) -> str: # функция удаляет символы, которые не буквы, не цифры, которые могут создать проблемы при переводе
...
marian_en_ru = Translator('en', 'ru') # экземпляр класса переводчика
...
class Pagem():  # класс подготовки страницы к выводу на страницу
    # global config
    def __init__(self, page) -> None:
        ...
        self.page = page
        self.text1 = page.get_text("dict")['blocks'] #формирует словарь из блоков тескта на странице page

        self.width_page = page.mediabox.width + 5      # получаем ширину и высоту исходной страницы
    ...
    def make_klc(self,):
        for number in self.text1:  # итерация по словарю
            text_ru = ''
            if 'lines' in number.keys():
                ls_number = number['lines']
                for lines in ls_number:
                    ls_lines = lines['spans']
                    for span in ls_lines:   # в этом цикле собираются разбитыечасти строки в одну целую стоку
                        if span['text'].strip() == '.':
                            self.klc[-1]['text'] = self.klc[-1]['text'].strip() + '.'
                            text_ru += '.'
                            continue
...
class MakePDF():
    ...
    def __init__(self, param: list, docum: Document, width: float, height: float):
        ...
        self.namefont = "figo"
        self.fontname = fitz.Font(fontname=self.namefont, fontfile=config.FONTFILE[0])
        self.fontsize = config.FONTSIZE[0]
    ...
    def rc_func(self, language, rect) -> float:
        rc = self.shape.insert_textbox(
            rect=rect,
            buffer=self.create_text(language),
    ...
    def make_paragraph(self,  add_right_down_y):
        ...
        if ("text_ru" in self.param.keys()):
            language = 'ru'
        rc = self.rc_func(language=language, rect=rect)
    ...
    def nnew_page(self):
        ...
        font = fitz.Font(fontname=self.namefont, fontfile=config.FONTFILE[0])
        self.page.insert_font(encoding=2, fontname=self.namefont, fontfile=config.FONTFILE[0], # fontsize=self.fontsize,  # 'figo',
...
file_document = config.UPLOAD_FOLDER[0] + '/' + 'Android.pdf'
doc = fitz.Document(file_document)
...
for tgg in doc.pages(start=86, stop=93):
    # tg = Pagem(doc[86])
    tg = Pagem(tgg)
    list_param = TakeList(listt=tg)

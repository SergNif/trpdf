...
class Pagem():
    ...
    def count_bbox(self, bbox_list:list) -> tuple:
        result = (bbox_list[0][0], bbox_list[0][1], bbox_list[-1][2], bbox_list[0][3])
        return result
        

    def join_stroke(self) -> None:
        global new_lst_gl
        # new_lst = []
        ...
        #         if 'type' not in el2:
        #             # print(f'{iix=}  {el2=}\n\n')  [(j['bbox'], j['text']) for j in self.klc if (('origin' in j) and (j['origin'][1]==y) and (y not in list_origin) and (j['text'] != "*ru-ru-ru*"))]
        ...
        for ix,el in enumerate(self.klc):
            ...
            if stroke not in el['text']:
                ...
                bbox_list = [j['bbox'] for j in self.klc if (('origin' in j) and (j['origin'][1]==y) and (y not in list_origin) and (j['text'] != "*ru-ru-ru*"))]
                
                bbox =  self.count_bbox(bbox_list=bbox_list)
                el['bbox'] = bbox
                list_origin.append(y)
                klc_list.append(el)

        with open("klc_list.txt", "a") as output:
            output.write(str(klc_list))
